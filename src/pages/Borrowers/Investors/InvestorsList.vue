<template>
  <div class="row mt-4">
    <div class="col-lg-4 order-lg-2">
      <!-- TODO: Convert the member search form to display sidebar style on mobile device -->
...
export default {
  ...
  methods: {
    async fetchInvestors () {
      try {
        this.end = false
        let { data } = await axios.get(`/api/users/investors/${this.page}`)
        console.log(data)
    ...
    async fetchOtherInvestors () {
      try {
        if (this.end === true) {
          ...
          this.loadMore = true
          console.log(`/api/users/investors/${this.page}`)
          let { data } = await axios.get(`/api/users/investors/${this.page}`)
          console.log(data)
          this.investors.push(...data.user)// Add the newly fetched user to the user collection that we have
          this.page = (data.current === data.pages) ? Number(data.current) : Number(data.current) + 1
